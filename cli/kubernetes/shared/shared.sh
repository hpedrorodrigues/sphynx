...
function sx::k8s::not_pending_pods() {
  ...
  sx::k8s::cli get pods \
    ...
    | while read -r templated_pod_line; do
      ...
      local ready_state="$(echo -e "${simple_pod_line}" | awk '{ print $2 }')"
      local pod_restarts_count="$(echo -e "${simple_pod_line}" | awk '{ print $4 }')"
      local pod_age="$(echo -e "${simple_pod_line}" | awk '{ print $5 }')"

      echo "${pod_namespace},${pod_name},${container_name},${ready_state},${pod_status},${pod_restarts_count},${pod_age},${description}"
    done \
    | column -t -s ','
}
