...
if name is not None:
    ...
    arrays = dict(
        (elem, value) for elem, value in args.items() if isinstance(value, list)
    )
    ...
    try:
        variables = dict(
            zip(map(name_mangle, args.keys()), map(to_bash, args.values()))
        )
    except ValueError as e:
        sys.exit(
            "%s: name could not be mangled into a valid Bash "
    ...
    if len(variables) < len(args):
        sys.exit(
            "%s: two or more elements have identically mangled names" % (sys.argv[0],)
        )
