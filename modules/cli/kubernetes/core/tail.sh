...
function sx::k8s::tail() {
  ...
  if sx::os::is_command_available 'fzf'; then
    local -r options="$(
      sx::k8s::shared::resources "${SX_KUBERNETES_TAIL_RESOURCES}" "${query}" "${namespace}" "${all_namespaces}" true
    )"
    ...
    if [ -n "${selected}" ]; then
      ...
      sx::k8s_command::tail "${ns}" "${kind}" "${name}" "${tool}" "${extra_flags}"
    fi
  else
    ...
    local options
    readarray -t options < <(
      sx::k8s::shared::resources "${SX_KUBERNETES_TAIL_RESOURCES}" "${query}" "${namespace}" "${all_namespaces}"
    )
...
function sx::k8s_command::tail() {
  ...
  if [ "${tool}" = 'kubetail' ]; then
    # shellcheck disable=SC2086  # quote this to prevent word splitting
    kubetail logs "${ns}:${resource}s/${name}" --follow ${extra_flags}
  elif [ "${kind}" = 'CronJob' ]; then
    # stern does not support cronjob resource queries, so match the
    # <cronjob>-<jobid>-<hash> pod names instead
    # shellcheck disable=SC2086  # quote this to prevent word splitting
    stern "^${name}-" --namespace "${ns}" ${extra_flags}
  else
    # shellcheck disable=SC2086  # quote this to prevent word splitting
    stern "${resource}/${name}" --namespace "${ns}" ${extra_flags}
  fi
}
